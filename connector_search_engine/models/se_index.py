...
class SeIndex(models.Model):
    ...
    obsolete_binding_ids = fields.One2many(
        comodel_name="se.index.obsolete.binding",
        inverse_name="index_id",
        string="Obsolete Binding IDs",
        readonly=True,
    )
    ...
    def _batch_delete_obsolete(self):
        self.ensure_one()
        obsolete_binding_ids = self.obsolete_binding_ids
        obsolete_binding_count = len(obsolete_binding_ids)
        while obsolete_binding_ids:
            processing = obsolete_binding_ids[0 : self.batch_size]  # noqa: E203
            obsolete_binding_ids = obsolete_binding_ids[self.batch_size :]  # noqa: E203
            description = _(
                "Delete %d obsolete records of %d for index '%s'",
                len(processing),
                obsolete_binding_count,
                self.name,
            )
            self.with_delay(description=description).delete_obsolete_item(
                processing.mapped("res_id")
            )
            processing.sudo().unlink()

    def batch_export(self, force_export=False):
        self.ensure_one()
        self._batch_export(force_export=force_export)
        self._batch_delete_obsolete()
        return True
